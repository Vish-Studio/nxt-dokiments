...
prefix. If that fails, use `dev`. Call the result `<base>`.
...
A key matches the pattern `[A-Z]+-[0-9]+` (e.g. `DOK-1234`). Gather candidates
...
is a primary key, append it in parentheses: `Add x feature (DOK-1234)`. Append
