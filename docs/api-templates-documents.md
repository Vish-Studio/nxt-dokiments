...
- **Request validation**: every route in these groups validates its input with a Zod schema via [`parseBody`/`parseQuery`](../src/lib/api/validate.ts) before touching Firestore — a malformed body (wrong types, missing fields) is rejected with `400` rather than reaching the Firestore layer. The same holds for [`/api/feedback`](./api-feedback.md) and `/api/newsletter`.
  **The `/api/auth/*` family is the exception**, and this note used to overstate the position by claiming otherwise. `POST /api/auth/update-profile` was brought in line (see [`profile-schema.ts`](../src/lib/api/profile-schema.ts)) because its unbounded `displayName` was reaching other features' Firestore rules and failing them. The remaining five — `sign-in`, `sign-up`, `update-password`, `reauthenticate`, `forgot-password` — still cast their bodies (`(await request.json()) as …`) unvalidated. Firebase itself rejects a bad email or a weak password, so the gap is narrower there than it looks, but it is a gap: `sign-up` in particular writes an unbounded `displayName` to Firebase Auth and to `users/{uid}`.
