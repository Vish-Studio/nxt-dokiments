...
- **Request validation**: every route in these groups validates its input with a Zod schema via [`parseBody`/`parseQuery`](../src/lib/api/validate.ts) before touching Firestore — a malformed body (wrong types, missing fields) is rejected with `400` rather than reaching the Firestore layer. The same holds for [`/api/feedback`](./api-feedback.md) and `/api/newsletter`.
  **The `/api/auth/*` family now holds to this too**, having been the long-standing exception. `POST /api/auth/update-profile` was brought in line first (see [`profile-schema.ts`](../src/lib/api/profile-schema.ts)) because its unbounded `displayName` was reaching other features' Firestore rules and failing them; the remaining five — `sign-in`, `sign-up`, `update-password`, `reauthenticate`, `forgot-password` — followed via [`auth-schema.ts`](../src/lib/api/auth-schema.ts). No route under `src/app/api/**` casts its body any more. Two things about that family are deliberate and worth knowing before editing it:
  - **It keeps its hand-rolled session handling and its own `catch` blocks** rather than moving to `withSession`. The response object has to exist before `saveSession` can attach a `Set-Cookie` to it, and these routes report a Firebase rejection with Firebase's own message for the form to render — which `handleApiError` would collapse into a generic `500`. Each one adds `if (error instanceof ApiError) return handleApiError(error)` so a rejected body keeps its own `400` instead. See the JSDoc on [`update-profile/route.ts`](../src/app/api/auth/update-profile/route.ts).
  - **A password is bounded only where one is being *set*** (`sign-up`, `update-password`). `sign-in` and `reauthenticate` verify a credential that already exists, and a ceiling there would permanently lock out the owner of a password longer than the limit. Nor does any schema enforce Firebase's six-character minimum: Firebase's own `WEAK_PASSWORD` message is more use to the user than "Invalid request body." See `credentialFieldLimits` in [`src/types/auth.ts`](../src/types/auth.ts).
